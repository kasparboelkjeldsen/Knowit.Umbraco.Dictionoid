...
{
    private readonly AzureAiApiConfiguration _config;
    private readonly OpenAIAPI _gptApi;

    public AzureAiClient(IOptions<AzureAiApiConfiguration> config)
    {
        _config = config.Value;
        _gptApi = OpenAIAPI.ForAzure(_config.ResourceId, _config.DeploymentId, _config.ApiKey);
        _gptApi.ApiVersion = _config.ApiVersion;
    }
    ...
    public async Task<ChatResult> TranslateAsync(List<ChatMessage> messages)
    {
        ...
        {
            Messages = messages,
            ResponseFormat = _config.DeploymentId.Contains("gpt-35")
                ? ChatRequest.ResponseFormats.Text
                : ChatRequest.ResponseFormats.JsonObject
        });
    }
